      ...
      <>
        { currPath==='/saved-news' ? 
          <section className="news-list news-list_type_saved page__wrapper">
            ...
            {
              savedArticles.map((cardElement, index) => <NewsCard key={index} card={cardElement} isLoggedIn={isLoggedIn} savedArticles={savedArticles} onDeleteArticle={onDeleteArticle}/>)
            }
            </div>
          </section>:    
